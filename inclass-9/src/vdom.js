...
function updateElement(parent, newNode, oldNode, index=0) {
    ...
    if (!oldNode) {
        ...
        if (changed(newNode, oldNode)) {
            var childEle = createElement(newNode); 
            parent.children.splice(index, 0, childEle);

        } else {
            if (newNode.children) {
                if (newNode.children < oldNode.children) {
                    var childEle = createElement(newNode); 
                    parent.replaceChild(childEle, parent.children[index])

                } else {
                     newNode.children.forEach(function(child, childIndex){
                        updateElement(parent.children[index], child, oldNode.children[childIndex], childIndex)
                    })
                }
            }
        }
